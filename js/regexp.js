/**
 * A RegExp extension that allows named capture groups in the syntax
 * /(?<name>regexp)/
 * 
...
/**
 * Extension of RegExp.exec()
 * Returns an extended array - first array element is matching string,
 * and elements thereafter are captured strings from regular (non-named)
 * groups. Named captures are extend upon arrays, e.g. for a name of
 * "date" the array will contain a property "date" with the captured string.
 *
 * Multiple matches behave like RegExp.exec(), where each iteration of the
 * call produces the next match, or null if there are no more matches.
 *
 * If there is no match for the regular expression, null is returned.
 * 
 * @param  {String} string test string
 * @return {Array}         array of match & captured matches, extended with
 *                         named capture groups as object properties
 */
